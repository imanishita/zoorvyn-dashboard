...
/**
 * Transaction modal — add or edit a transaction.
 * Auto-switches available categories based on selected type (Income vs Expense).
 */
export default function TransactionModal({ initialData, onClose }) {
  ...
  // Auto-update category when type changes (reset to first option of new type)
  useEffect(() => {
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4">
            ...
            <div className="pt-4 flex justify-end gap-3">
              <button
                type="button"
                onClick={onClose}
                className="rounded-xl px-4 py-2 text-sm font-medium text-gray-600 transition-all duration-200 hover:bg-gray-100 hover:text-slate-900 dark:text-gray-300 dark:hover:bg-slate-800 dark:hover:text-white"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="rounded-xl bg-brand-500 px-4 py-2 text-sm font-medium text-white shadow-md transition-all duration-200 hover:bg-brand-600 hover:shadow-lg active:scale-[0.98]"
              >
